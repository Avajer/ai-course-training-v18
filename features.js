...
(function () {
  ...
  var PROMPT_ELEMENTS = [
    { key: "role", name: "Роль", hint: "Кем должна выступать модель: «Ты редактор деловых текстов…», «Действуй как аналитик…».",
      re: /(ты\s+(?:опытн[а-яё]*\s+)?(?:эксперт|аналитик|редактор|юрист|специалист|консультант|преподавател|методист|менеджер|аудитор|инспектор|секретар|копирайтер|маркетолог|ассистент|помощник|hr|рекрут)|выступ(?:и|ай)\s+как|в\s+рол[ие]\s|действуй\s+как|представь,?\s+что\s+ты)/i },
    { key: "task", name: "Задача", hint: "Чёткое действие-глагол: проанализируй, составь, проверь, сравни, перепиши, резюмируй.",
      re: /(проанализируй|составь|напиши|сделай|проверь|сравни|подготов[ьи]|оцени|сформируй|выдели|перепиши|резюмируй|сократи|переведи|найди|разбер[иё]|объясни|предложи|сгенерируй|структурируй|классифицируй|извлеки|верни)/i },
    { key: "context", name: "Контекст / цель", hint: "Зачем это нужно и для кого: «цель — …», «аудитория — руководитель», «ситуация: …».",
      re: /(цель[ью:\s]|контекст|ситуац|аудитори|адресат|для\s+(?:кого|руководител|клиент|совещан|отчёт|отчет)|мне\s+нужно|нам\s+нужно|у\s+меня\s+есть|задача\s+проекта|чтобы\s+)/i },
    { key: "data", name: "Исходные данные", hint: "Дай материал или место для него: «текст ниже», «документ: …» или поле в [квадратных скобках].",
      re: /(\[[^\]]{2,}\]|ниже|следующ(?:ий|ем|его)\s+(?:текст|документ|данны)|документ:|текст:|данные:|вставь|приложен|на\s+основе\s+(?:текста|данных|документа))/i },
    { key: "format", name: "Формат результата", hint: "Как оформить ответ: таблица, список, по пунктам, отчёт, JSON, шаблон письма.",
      re: /(формат|в\s+виде|таблиц|спис(?:ок|ком|ка)|по\s+пунктам|пунктам|структур|markdown|json|отчёт|отчет|нумерован|маркирован|шаблон|колонк|раздел[аыи])/i },
    { key: "style", name: "Стиль / тон", hint: "Тон и язык: деловой, нейтральный, кратко, простыми словами, официальный.",
      re: /(стил[ье]|тон[ае]?[:\s]|делов|официальн|нейтральн|кратк|лаконичн|простыми\s+словами|формальн|дружелюбн|сдержанн|без\s+воды)/i },
    { key: "limits", name: "Ограничения / критерии", hint: "Рамки и контроль фактов: «не придумывай», «до 300 слов», «только проверенное», критерии качества.",
      re: /(не\s+(?:придумыв|выдумыв|добавляй|фантазир)|если\s+данных\s+недостаточно|ограничь|объ[её]м|до\s+\d+\s*(?:слов|символов|предложен|строк)|не\s+более|только\s+|без\s+|критери|проверь\s+факт|раздели\s+факт|укажи\s+(?:допущен|что\s+нужно\s+уточнить))/i }
  ];
