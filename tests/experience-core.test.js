...
test("логотип и аккаунт занимают отдельную адаптивную строку над видео", () => {
  const page = fs.readFileSync(new URL("../index.html", import.meta.url), "utf8");
  const styles = fs.readFileSync(new URL("../experience.css", import.meta.url), "utf8");
  const baseStyles = fs.readFileSync(new URL("../styles.css", import.meta.url), "utf8");
  const course = fs.readFileSync(new URL("../script.js", import.meta.url), "utf8");

  assert.match(page, /class="hero-topbar"[\s\S]*id="brandLogoHero"[\s\S]*id="accountStatus"[\s\S]*class="hero-video-cover"/);
  assert.match(styles, /\.hero-topbar\s*\{[\s\S]*display:\s*flex/);
  assert.match(styles, /#courseHero\.hero-slide\s*>\s*\.hero-topbar\s*\{[\s\S]*z-index:\s*[3-9]/);
  assert.match(styles, /\.hero-topbar\s+\.account-status--hero\s*\{[\s\S]*position:\s*static/);
  assert.match(styles, /\.hero-topbar\s+\.account-popover\s*\{[\s\S]*position:\s*static/);
  assert.match(styles, /@media \(max-width:\s*760px\)[\s\S]*\.hero-topbar\s*\{[\s\S]*flex-direction:\s*column/);
  assert.match(baseStyles, /\[hidden\]\s*\{\s*display:\s*none\s*!important;?\s*\}/);
  assert.match(course, /if \(accountMenuPanel\)\s*\{[\s\S]*accountChip\?\.addEventListener\("click"/);
});
...
test("видеообложка адаптивна и останавливается при уменьшении движения", () => {
  ...
  assert.match(styles, /\.hero-video-cover__media\s*\{[\s\S]*position:\s*absolute[\s\S]*inset:\s*0/);
});
...
test("номер сборки синхронизирован в странице, логике и автономном кэше", () => {
  const page = fs.readFileSync(new URL("../index.html", import.meta.url), "utf8");
  const course = fs.readFileSync(new URL("../script.js", import.meta.url), "utf8");
  const features = fs.readFileSync(new URL("../features.js", import.meta.url), "utf8");
  const worker = fs.readFileSync(new URL("../sw.js", import.meta.url), "utf8");
  const pageVersion = page.match(/styles\.css\?v=(\d+)/)?.[1];
  const courseVersion = course.match(/COURSE_BUILD\s*=\s*"v(\d+)"/)?.[1];
  const featuresVersion = features.match(/COURSE_VERSION\s*=\s*"v(\d+)"/)?.[1];
