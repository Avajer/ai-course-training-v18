...
test("trainer history is excluded from course submissions and server payloads", () => {
  const course = read("script.js");
  const server = read("google-apps-script.js");

  assert.doesNotMatch(course, /trainerHistory|aiCoursePromptTrainerHistoryV1/);
  assert.doesNotMatch(server, /trainerHistory|aiCoursePromptTrainerHistoryV1/);
});
